...
The first screen after opening the demo shows the populated approval gates.
...
normal storage uses `send-gate-local` and never reads or writes the demo namespace.
...
- **Start for real** — leaves `/demo` for empty normal storage. Demo records
  remain isolated and are not copied into it.
